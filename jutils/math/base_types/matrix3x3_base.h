...
namespace jutils
{
    namespace math
    {
        ...
        public:
            ...
            {
                return JSTR("{ ") + rows[0].toString() + JSTR("; ") + rows[1].toString() + JSTR("; ") + rows[2].toString() + JSTR(" }");
            }
        ...
        template<typename Type>
        template<typename OtherType>
        constexpr matrix<3, 3, Type>& matrix<3, 3, Type>::operator+=(const matrix<3, 3, OtherType>& value)
        {
            rows[0] += value.rows[0];
            rows[1] += value.rows[1];
            rows[2] += value.rows[2];
        ...
        constexpr matrix<3, 3, Type>& matrix<3, 3, Type>::operator*=(const OtherType& value)
        {
