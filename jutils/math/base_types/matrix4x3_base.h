...
namespace jutils
{
    namespace math
    {
        ...
        public:
            ...
            constexpr matrix(const type x1, const type y1, const type z1, const type x2, const type y2, const type z2, const type x3, const type y3, const type z3, const type x4, const type y4, const type z4)
                : rows{ row_type(x1, y1, z1), row_type(x2, y2, z2), row_type(x3, y3, z3), row_type(x4, y4, z4) }
            {}
            constexpr matrix(const row_type& row1, const row_type& row2, const row_type& row3, const row_type& row4)
                : rows{ row1, row2, row3, row4 }
            {}
            template<typename OtherType>
            constexpr matrix(const matrix<4, 3, OtherType>& value)
            ...
            {}
            template<vector_size_type ColumnsCount, typename OtherType, TEMPLATE_ENABLE((ColumnsCount != 3))>
            ...
            constexpr column_type getColumn(const vector_size_type index) const
            {
                ...
                return column_type(rows[0].z, rows[1].z, rows[2].z, rows[3].z);
            }

            template<typename OtherType>
            constexpr matrix& operator+=(const matrix<4, 3, OtherType>& value);
            template<typename OtherType>
            constexpr matrix& operator-=(const matrix<4, 3, OtherType>& value);
            ...
            template<typename OtherType>
            constexpr bool operator==(const matrix<4, 3, OtherType>& value) const
            {
                return (rows[0] == value.rows[0]) && (rows[1] == value.rows[1]) && (rows[2] == value.rows[2]) && (rows[3] == value.rows[3]);
            }
            template<typename OtherType>
            constexpr bool operator!=(const matrix<4, 3, OtherType>& value) const { return !this->operator==(value); }

            jstring toString() const { return JSTR("{ ") + rows[0].toString() + JSTR("; ") + rows[1].toString() + JSTR("; ") + rows[2].toString() + JSTR("; ") + rows[3].toString() + JSTR(" }"); }

            constexpr transpose_type transpose() const { return { rows[0].x, rows[1].x, rows[2].x, rows[3].x, rows[0].y, rows[1].y, rows[2].y, rows[3].y, rows[0].z, rows[1].z, rows[2].z, rows[3].z }; }
        };

        template<typename Type>
        template<typename OtherType>
        constexpr matrix<4, 3, Type>& matrix<4, 3, Type>::operator+=(const matrix<4, 3, OtherType>& value)
        {
            rows[0] += value.rows[0];
            rows[1] += value.rows[1];
            rows[2] += value.rows[2];
        ...
        constexpr matrix<4, 3, Type>& matrix<4, 3, Type>::operator*=(const OtherType& value)
        {
            rows[0] *= value;
            rows[1] *= value;
            rows[2] *= value;
