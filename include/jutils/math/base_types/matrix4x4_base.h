...
namespace jutils
{
    ...
    {
        template<typename T>
        class matrix<4, 4, T> : public _matrix4_base<4, T>
        {
            using base_type = _matrix4_base<4, T>;
        public:
            ...
            explicit constexpr matrix(const type value) noexcept
                : base_type(row_type(value, 0, 0, 0), row_type(0, value, 0, 0), row_type(0, 0, value, 0), row_type(0, 0, 0, value))
            {}
            constexpr matrix(const type x1, const type y1, const type z1, const type w1, 
                             ...
                             const type x4, const type y4, const type z4, const type w4) noexcept
                : base_type(row_type(x1, y1, z1, w1), row_type(x2, y2, z2, w2), row_type(x3, y3, z3, w3), row_type(x4, y4, z4, w4))
            {}
