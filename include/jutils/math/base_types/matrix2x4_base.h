...
{
    template<typename T>
    class matrix<2, 4, T> : public _matrix2_base<4, T>
    {
        using base_type = _matrix2_base<4, T>;
    ...
    public:
        ...
        constexpr matrix(const type x1, const type y1, const type z1, const type w1,
                         const type x2, const type y2, const type z2, const type w2) noexcept
            : base_type(row_type(x1, y1, z1, w1), row_type(x2, y2, z2, w2))
        {}
