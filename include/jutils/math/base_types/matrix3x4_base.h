...
namespace jutils
{
    ...
    {
        template<typename T>
        class matrix<3, 4, T> : public _matrix3_base<4, T>
        {
            using base_type = _matrix3_base<4, T>;
        public:
            ...
            constexpr matrix(const type x1, const type y1, const type z1, const type w1, 
                             ...
                             const type x3, const type y3, const type z3, const type w3) noexcept
                : base_type(row_type(x1, y1, z1, w1), row_type(x2, y2, z2, w2), row_type(x3, y3, z3, w3))
            {}
