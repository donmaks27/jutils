...
namespace jutils
{
    ...
    private:
        ...
        static auto _helper(int32, T1) -> decltype(std::declval<jutils::hash<T1>>()(std::declval<T1>()));
        template<typename T1>
        static void _helper(int8, T1);
    public:
        static constexpr bool value = std::is_same_v<decltype(_helper(0, std::declval<T>())), hash_type>;
    };
    template<typename T>
    ...
    namespace math::hash
    {
        ...
        [[nodiscard]] constexpr uint64 crc64(const uint8* data, const uint64 length) noexcept
        {
        ...
        [[nodiscard]] constexpr uint64 crc64(const char* str, const uint64 length) noexcept
        {
            constexpr uint64 mask = 0x00000000000000FF;
        ...
        [[nodiscard]] constexpr uint64 crc64(const char* str) noexcept
            { return str != nullptr ? crc64(str, std::char_traits<char>::length(str)) : 0; }

        [[nodiscard]] constexpr uint64 crc64(const uint8 value) noexcept { return crc64_table[value]; }
        [[nodiscard]] constexpr uint64 crc64(const uint16 value) noexcept
        {
            constexpr uint64 mask = 0x00000000000000FF;
        ...
        [[nodiscard]] constexpr uint64 crc64(const uint32 value) noexcept
        {
            constexpr uint64 mask = 0x00000000000000FF;
        ...
        [[nodiscard]] constexpr uint64 crc64(const uint64 value) noexcept
        {
            constexpr uint64 mask = 0x00000000000000FF;
        ...
        [[nodiscard]] constexpr uint64 crc64(const T value) noexcept { return crc64(static_cast<std::make_unsigned_t<T>>(value)); }
    }
}
...
{
    [[nodiscard]] constexpr jutils::hash_type operator()(const char* str) const noexcept { return jutils::math::hash::crc64(str); }
};
